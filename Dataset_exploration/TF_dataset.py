...
ax = plt.hist(TF_expressions.to_numpy().flatten())
fig.suptitle('TF expression values distribution')

print(f'There are {len(TF_expressions.to_numpy().flatten())} expression values in the dataset')

plt.savefig('../figures/TF_express_dist')